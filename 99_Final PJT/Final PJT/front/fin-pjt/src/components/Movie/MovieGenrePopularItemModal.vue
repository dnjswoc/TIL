      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="poster-container">
            <img :src="posterUrl + store.movieDetail.poster_path" alt="poster" class="poster-img" />
          </div>
                ...
                <div class="movie-details">
                  ...
                  <hr>
                  <p><strong>개봉일 : </strong> {{ store.movieDetail.release_date }}</p>
                  <p><strong>상영시간 : </strong>{{ formattedRuntime }}</p>
                  <p><strong>장르 : </strong> {{ store.movieDetail.genre1 }} / {{ store.movieDetail.genre2 }}</p>
                  <MovieGenrePopularItemYoutube />
                </div>
        ...
        <div class="comment-section">
          <MovieCommentForm 
          :movieId="store.movieDetail.movie_id"
          />
        </div>
      </div>
    </div>
...
const like_message = ref('좋아요')

const isLiked = ref((store.movieDetail.like_users.indexOf(accountStore.userId.id)) > 0 ? true : false)


// 좋아요 버튼 작동
const toggleLike = () => {
  isLiked.value = !isLiked.value;
  likeMovie(store.movieDetail.movie_id);
};
...
const likeMovie = function (id) {
  ...
  .then((res) => {
    ...
    store.getMovieDetail(store.movieDetail.movie_id)
  })
  .catch((err) => {
    console.log(err)
  })
...
const likeUsersLength = computed(() => {
  return store.movieDetail.like_users.length
})

// 좋아요한 유저 id
const likeUsers = computed(() => {
  return store.movieDetail.like_users
})

// 상영시간을 "시간과 분" 형식으로 변환
const formattedRuntime = computed(() => {
  const runtime = store.movieDetail.runtime; // 상영시간(분)
