...
def edit_todo(request, todo_pk):
    edited_todo = Todo.objects.get(pk=todo_pk)
    edited_todo.work = request.POST.get('work')
    edited_todo.content = request.POST.get('content')
    edited_todo.save()
    return redirect('todos:detail', edited_todo.pk)